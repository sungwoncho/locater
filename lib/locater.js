 ...
 * @param line {String} - the string to remove line break from
 */
 ...
 * @param line {String} - the string split
 */
...
module.exports = {
  find: function (target, source) {
    ...
    if (!(target instanceof Array)) target = [target] //convert all targetss to array

    sourceLines.forEach(function (line, lineNumber) {
      target.forEach(function(currentValue) {
        var indices = findIndices(currentValue, line);

        indices.forEach(function (index) {
          results.push( { line: lineNumber + 1, cursor: index + 1 } );
        });
      })
    });
